...
export default () => (
  ...
  >
    ...
    <Space>
      ...
      <Button type="button" color="secondary">注册</Button>
    </Space>
  </Form>
);
```
...
export default () => (
  <Stack gap={3} className="max-w-md w-full">
    <Text muted size="sm">默认 md — 单行表单控件统一 32px 高度</Text>
...
`Form.useWatch` 订阅字段值；校验失败时默认滚动并聚焦第一个错误项（可用 `scrollToFirstError={false}` 关闭）。
...
function NicknameHint() {
  ...
  return (
    <Text muted className="text-sm">
      预览：{nickname ? String(nickname) : '尚未填写'}
    </Text>
  );
}
