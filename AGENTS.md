...
- LLM docs: generated on CI only — `doc:build` and release `prepare` run `pnpm llm:generate` (writes `docs/public/` + `packages/core/{llms.txt,llms-full.txt,registry.json}`). Do **not** commit these files; local push of source is enough.
